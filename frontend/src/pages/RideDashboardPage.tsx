...
function getSimulatedBucketStats(
  ...
  if (algorithm === "token-bucket") {
    return {
      ...
      refillRate: params.tokenBucket.refillRate,
      utilization: (
        ((params.tokenBucket.capacity - step.tokenBucket.tokens) /
  ...
  return {
    ...
    refillRate: params.leakyBucket.leakRate,
    utilization: (
      (step.leakyBucket.queueSize / params.leakyBucket.capacity) *
      100
    ).toFixed(2),
    currentLevel: step.leakyBucket.queueSize,
    leakRate: params.leakyBucket.leakRate,
    queueRemaining: Math.max(
      0,
      params.leakyBucket.capacity - step.leakyBucket.queueSize,
    ),
  };
}
...
export default function RideDashboardPage() {
  ...
  const [tutorialBurstInjected, setTutorialBurstInjected] = useState(false);
  const [tutorialPredictionToggled, setTutorialPredictionToggled] =
    useState(false);
  const [tutorialModeChanged, setTutorialModeChanged] = useState(false);
  const [tutorialTabVisited, setTutorialTabVisited] = useState<{
  ...
  const handleToggleToolbox = useCallback(() => {
    setToolboxOpen((previous) => {
      const next = !previous;
      if (next) {
        setConfigDraft((draftPrevious) =>
          buildDraftFromLiveStats(draftPrevious),
        );
      }
      return next;
    });
  ...
  }, [rideState, tokenBucket, slidingWindow, leakyBucket, simulation]);
  ...
  const tutorialContext: TutorialContext = {
    mode: simulation.mode,
    cursorIndex: simulation.cursorIndex,
    selectedAlgo,
    eventsLength: simulation.events.length,
    baseRequestCount: rideState?.rideCount ?? 0,
    tokenTokens: simulation.currentStep?.tokenBucket.tokens ?? 0,
    slidingCount: simulation.currentStep?.slidingWindow.count ?? 0,
    leakyQueue: simulation.currentStep?.leakyBucket.queueSize ?? 0,
    hasDivergence:
      simulation.divergentRequestIds.size > 0 ||
      Boolean(simulation.currentStep?.isDecisionDifferent),
    predictionVisible,
    hasPrediction: Boolean(simulation.prediction),
    burstInjectedInTutorial: tutorialBurstInjected,
    predictionToggledInTutorial: tutorialPredictionToggled,
    modeChangedInTutorial: tutorialModeChanged,
    tabVisitedInTutorial: tutorialTabVisited,
  };

  const tutorial = useTutorialGuide({
  ...
  const handleStartTutorial = useCallback(() => {
    ...
    setIsRaceMode(false);
    setSelectedAlgo("token-bucket");
    setPredictionVisible(true);
    setTutorialBurstInjected(false);
    setTutorialPredictionToggled(false);
    setTutorialModeChanged(false);
    setTutorialTabVisited({
      "token-bucket": false,
      "sliding-window": false,
      "leaky-bucket": false,
    });
    tutorial.start();
  }, [hasProcessed, showAdvancedView, tutorial]);
  ...
  }, [selectedAlgo, configDraft, tokenBucket, slidingWindow, leakyBucket]);
  ...
  const handleScenarioSelect = useCallback(
    (scenario: "flash-sale" | "ddos" | "steady") => {
      ...
      Promise.all([
        tokenBucket.updateConfig({
          capacity: newDraft.tb_capacity,
          refillRate: newDraft.tb_refillRate,
        }),
        slidingWindow.updateConfig({
          maxRequests: newDraft.sw_maxRequests,
          windowSize: newDraft.sw_windowSize,
        }),
        leakyBucket.updateConfig({
          capacity: newDraft.lb_capacity,
          leakRate: newDraft.lb_leakRate,
        }),
      ])
        .then(() => {
          return Promise.all([
  ...
  const selectedRequestStats = hasProcessed
    ? getSimulatedRequestStats(
        selectedAlgo,
        simulation.steps.slice(0, simulation.cursorIndex),
      )
  ...
  return (
    <div className="page page-dashboard">
      ...
      {hasProcessed && (
        <section className="dashboard-view-toggle">
          <button
            type="button"
            className="btn btn-outline"
            onClick={handleStartTutorial}
          >
            {tutorial.isFinished ? "Restart Tutorial" : "Start Tutorial"}
          </button>
          <button
            type="button"
            className={`btn ${showAdvancedView ? "btn-primary" : "btn-outline"}`}
            onClick={() => setShowAdvancedView((previous) => !previous)}
          >
      ...
      {hasProcessed && showAdvancedView && (
        <section className="advanced-view-grid">
          <div className="advanced-view-main">
            <section className="simulation-shell card">
              ...
              <SimulationControls
                ...
                onBurst={() => {
                  if (tutorial.isActive) {
                    setTutorialBurstInjected(true);
                  }
                  simulation.addBurst(6);
                }}
                onSpeedChange={simulation.setSpeed}
                onTogglePrediction={() => {
                  if (tutorial.isActive) {
                    setTutorialPredictionToggled(true);
                  }
            ...
            <section className="algo-tabs-container">
              <button
                ...
                onClick={() => {
                  ...
                  setSelectedAlgo("token-bucket");
                  if (tutorial.isActive) {
                    setTutorialTabVisited((previous) => ({
                      ...previous,
                      "token-bucket": true,
                    }));
                  }
                }}
                data-tutorial="tab-token-bucket"
              >
                Token Bucket
              </button>
              <button
                ...
                onClick={() => {
                  setIsRaceMode(false);
                  setSelectedAlgo("sliding-window");
                  if (tutorial.isActive) {
                    setTutorialTabVisited((previous) => ({
                      ...previous,
                      "sliding-window": true,
                ...
                data-tutorial="tab-sliding-window"
              >
                Sliding Window
              </button>
              <button
                ...
                data-tutorial="tab-leaky-bucket"
              >
                Leaky Bucket
              </button>
              <div
            ...
            {isRaceMode ? (
              ...
              >
                <div className="card">
                  ...
                  <BucketView
                    current={
                      hasProcessed && simulation.currentStep
                        ? simulation.currentStep.tokenBucket.tokens
                        : (tokenBucket.bucketStats?.tokens ?? 0)
                    }
                    capacity={activeSimulationParams.tokenBucket.capacity}
                    refillRate={activeSimulationParams.tokenBucket.refillRate}
                    algorithm="token-bucket"
                    lastRequestSuccess={
                      hasProcessed
                        ? (simulation.currentStep?.tokenBucket.accepted ?? null)
                        : tbLastSuccess
                    }
                  />
                </div>
                <div className="card">
                  ...
                  <WindowView
                    currentCount={
                      hasProcessed && simulation.currentStep
                        ? simulation.currentStep.slidingWindow.count
                        : (slidingWindow.bucketStats?.currentCount ?? 0)
                    }
                    maxRequests={
                      activeSimulationParams.slidingWindow.maxRequests
                    }
                    windowSize={activeSimulationParams.slidingWindow.windowSize}
                    lastRequestSuccess={
                      hasProcessed
                ...
                <div className="card">
                  ...
                  <LeakyBucketView
                    currentLevel={
                      hasProcessed && simulation.currentStep
                        ? simulation.currentStep.leakyBucket.queueSize
                        : (leakyBucket.bucketStats?.currentLevel ?? 0)
                    }
                    capacity={activeSimulationParams.leakyBucket.capacity}
                    leakRate={activeSimulationParams.leakyBucket.leakRate}
                    lastRequestSuccess={
                      hasProcessed
              ...
              <section className="workbench-layout single-algo-layout">
                ...
                <section className="algo-comparison single-panel">
                  <AlgorithmPanel
                    algorithm={selectedAlgo}
                    requestStats={selectedRequestStats}
                    bucketStats={
                      hasProcessed
                        ? getSimulatedBucketStats(
                            selectedAlgo,
                            simulation.steps,
                            simulation.cursorIndex,
                    ...
                    lastRequestSuccess={
                      hasProcessed
                        ? ((selectedAlgo === "token-bucket"
                            ...
                            : selectedAlgo === "sliding-window"
                              ? simulation.currentStep?.slidingWindow.accepted
                              : simulation.currentStep?.leakyBucket.accepted) ??
                          null)
                        : selectedAlgo === "token-bucket"
                          ? tbLastSuccess
                          : selectedAlgo === "sliding-window"
                ...
                <aside className="code-rail code-rail-right">
                  {predictionVisible && simulation.prediction && (
                    <section className="prediction-card card" data-tutorial="prediction-card">
                      <h3>Prediction Mode</h3>
                      <p className="prediction-subtitle">
                        Next request #{simulation.prediction.request.id} at t=
                        {(simulation.prediction.request.time / 1000).toFixed(2)}
                        s
                      </p>
                      ...
                      {simulation.prediction.isDecisionDifferent && (
                        <p className="prediction-alert">
                          Algorithms are expected to diverge on the next
                          request.
                        </p>
                      )}
                    </section>
                  ...
                  <section className="card card-stats advanced-side-stats">
                    <h3>Statistics</h3>
                    {selectedRequestStats ? (
                      <StatsPanel
                        total={selectedRequestStats.total}
                        allowed={selectedRequestStats.allowed}
                        blocked={selectedRequestStats.blocked}
                        successRate={selectedRequestStats.successRate}
                        lastRequestSuccess={
                          hasProcessed
                            ? ((selectedAlgo === "token-bucket"
                                ? simulation.currentStep?.tokenBucket.accepted
            ...
            <div className="comparison-table">
              ...
              <div className="comparison-row">
                ...
                <span className="stat-success">
                  {tokenBucket.requestStats.allowed}
                </span>
                <span className="stat-success">
                  {slidingWindow.requestStats.allowed}
                </span>
                <span className="stat-success">
                  {leakyBucket.requestStats.allowed}
                </span>
              </div>
              <div className="comparison-row">
                <span>Bookings Rejected</span>
                <span className="stat-danger">
                  {tokenBucket.requestStats.blocked}
                </span>
                <span className="stat-danger">
                  {slidingWindow.requestStats.blocked}
                </span>
                <span className="stat-danger">
                  {leakyBucket.requestStats.blocked}
                </span>
              </div>
              <div className="comparison-row">
              ...
              <div className="comparison-row">
                ...
                <span>
                  INR {tokenBucket.requestStats.allowed * rideState.farePerRide}
                </span>
                <span>
                  INR{" "}
                  {slidingWindow.requestStats.allowed * rideState.farePerRide}
                </span>
                <span>
                  INR {leakyBucket.requestStats.allowed * rideState.farePerRide}
                </span>
              </div>
            </div>
          </section>
      ...
      {hasProcessed && requestLog.length > 0 && !showAdvancedView && (
      ...
      <TutorialOverlay
        open={tutorial.isActive && hasProcessed && showAdvancedView}
        step={tutorial.currentStep}
        stepIndex={tutorial.stepIndex}
        totalSteps={tutorial.totalSteps}
        canGoBack={tutorial.canGoBack}
        canGoNext={tutorial.canGoNext}
        isStepComplete={tutorial.isCurrentStepComplete}
